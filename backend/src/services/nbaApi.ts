  ...
  const doFetch = async () => {
    ...
    try {
      const res = await fetch(url, {
        method: "GET",
        headers: BROWSER_HEADERS as any,
      ...
      if (err?.name === "AbortError") {
        throw new Error(
          `NBA API timeout after ${NBA_API_TIMEOUT_MS}ms on ${endpoint}`
        );
      }
      throw err;
    } finally {
...
export async function getAllModernPlayers(): Promise<NbaPlayerMeta[]> {
