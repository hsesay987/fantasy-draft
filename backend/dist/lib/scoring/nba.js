...
exports.baseNbaScore = baseNbaScore;
exports.advancedNbaScore = advancedNbaScore;
exports.applyEraAdjustment = applyEraAdjustment;
exports.calculateCompatibilityValue = calculateCompatibilityValue;
exports.scoreNbaPlayer = scoreNbaPlayer;
exports.normalizeFranchise = normalizeFranchise;
function clamp(x, min, max) {
    return Math.max(min, Math.min(max, x));
}
function normalize(value, elite) {
    return clamp(value / elite, 0, 1);
}
/* -------------------------------------------------------------------------- */
/* 1. BASE VALUE – “Box Score” Offense & Defense (0–60)                       */
/* -------------------------------------------------------------------------- */
function baseNbaScore(stat, position) {
    // Offense core
    ...
    const efficiency = stat.tsPct != null
        ? clamp((stat.tsPct - 0.5) / (0.66 - 0.5), 0, 1) // .50 = avg, .66 = god tier
        : 0.5;
    const spacing = stat.threeRate != null ? clamp(stat.threeRate / 0.6, 0, 1) : 0; // ~0.6 = pure gunner
...
/* -------------------------------------------------------------------------- */
function advancedNbaScore(stat) {
...
/* -------------------------------------------------------------------------- */
function applyEraAdjustment(score, stat, era) {
    if (!era.eraFrom || !era.eraTo)
        return score;
...
/* -------------------------------------------------------------------------- */
function calculateCompatibilityValue(stat, fit) {
    let comp = 0;
    ...
    if (currentPosCount === 0)
        comp += 4; // filling a need
    else if (currentPosCount === 1)
        comp += 1; // fine
    else if (currentPosCount >= 2)
        comp -= 3 * (currentPosCount - 1); // overcrowded
    // b) Height fit
    const teamAvgHeight = fit.teamHeights.length > 0
    ...
    // c) Usage fit
    const teamAvgUsage = fit.teamUsage.length > 0
...
function accoladeBonus(stat) {
    ...
    if (stat.allNBA != null) {
        if (stat.allNBA === 1)
            bonus += 4;
        else if (stat.allNBA === 2)
            bonus += 3;
        else if (stat.allNBA === 3)
            bonus += 2;
    }
...
/* -------------------------------------------------------------------------- */
function scoreNbaPlayer(stat, position, era, fit) {
